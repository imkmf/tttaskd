...
class OverdueContainer extends React.Component {
  componentWillMount () {
    const {
      tasksRequested,
      projectsRequested,
  ...
  render () {
    ...
    const filteredTasks = tasks.filter(t => !t.completed && moment(t.due_at).isBefore(moment()))
    ...
    return (
      <div>
        ...
        <div className={cx(classes.topPad, ' ui masthead vertical segment ', { leftPad: sidebarExpanded })}>
          <h1 className="ui header">Overdue</h1>
...
export default connect(mapStateToProps, mapActionCreators)(OverdueContainer)
